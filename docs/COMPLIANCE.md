...
| 6 | Open-source technologies only, share working source, no extra licenses | **[x]** | MobileFaceNet (MIT), MediaPipe BlazeFace + FaceLandmarker (Apache-2.0), MiniFASNet (Apache-2.0), all RN libraries MIT. Full source in this repo. License map in `docs/MODEL_PIPELINE.md`. NetraID itself is MIT licensed (`LICENSE` at the repository root), so it can be embedded in Datalake 3.0, modified and redistributed without restriction; see `THIRD-PARTY-NOTICES.md` for every third-party component and its licence |
