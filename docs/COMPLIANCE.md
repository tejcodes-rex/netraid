...
| 6 | Open-source technologies only, share working source, no extra licenses | **[x]** | MobileFaceNet (MIT), MediaPipe BlazeFace + FaceLandmarker (Apache-2.0), MiniFASNet (Apache-2.0), all RN libraries MIT. Full source in this repo. License map in `docs/MODEL_PIPELINE.md` |
...
- **[x]** Two calibrated liveness layers, both attacked and verified on the target device. Active: blink is mandatory in every random challenge set (a photo cannot blink), blink evidence is depth-weighted and only accumulates on a motion-stable head (defeats blur-faked closures from swinging a photo), smile/turn must hold 2 frames, and challenge/capture timeouts re-randomize the set (anti-replay). Passive: MiniFASNet float32 gated at max P(real) >= 0.08 across the verify captures, calibrated on-device (live face 0.12-0.74 vs laptop-screen replay of the enrolled user <= 0.04). Combined in `decideLiveness()`; a screen replay of the ENROLLED user is rejected, measured live.
...
|---|:---:|---|
| Innovation, edge AI efficiency + compression + liveness | 30 | robust float32 embedder chosen after real on-device testing (int8 produced NaN, fp16 stalled the interpreter on mid-range hardware); whole stack ≈ 17 MB, under the 20 MB cap; two calibrated anti-spoof layers (mandatory-blink FSM + device-calibrated MiniFASNet gate); multi-frame flip-TTA accuracy engine with duplicate-face guard; embeddings-only privacy model; 371-457 ms full multi-frame verdict on a real mid-range phone |
