...
| 6 | Open-source technologies only, share working source, no extra licenses | **[x]** | MobileFaceNet (MIT), MediaPipe BlazeFace + FaceLandmarker (Apache-2.0), MiniFASNet (Apache-2.0), all RN libraries MIT. Full source in this repo. License map in `docs/MODEL_PIPELINE.md`. NetraID is released under the Apache License 2.0 (`LICENSE` at the repository root), so NHAI can deploy, integrate and modify it with no licence fee and no per-device cost, with an express patent grant; `THIRD-PARTY-NOTICES.md` lists every third-party component and its licence |
